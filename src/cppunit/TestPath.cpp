...
TestPath::TestPath(const TestPath& other, int indexFirst, int count)
    : m_tests()
{
...
TestPath::TestPath(Test* searchRoot, const std::string& pathAsString)
    : m_tests()
{
...
void TestPath::insert(Test* test, int index)
{
...
void TestPath::insert(const TestPath& path, int index)
{
...
Test* TestPath::findActualRoot(Test* searchRoot, const std::string& pathAsString, PathTestNames& testNames)
{
	...
	Test* root = isRelative ? searchRoot->findTest(testNames[0]) : searchRoot;
...
bool TestPath::splitPathString(const std::string& pathAsString, PathTestNames& testNames)
{
	...
	std::string::size_type index = (isRelative ? 0 : 1);
	for(;;)
	{
		std::string::size_type separatorIndex = pathAsString.find_first_of("/:", index);
		...
		else
		{
			...
			if(pathAsString[separatorIndex] == '/')
				index = separatorIndex + 1;
			else
				index = separatorIndex + 2;
		}
	}
