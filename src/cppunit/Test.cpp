...
Test* Test::getChildTestAt(int index) const
{
...
Test* Test::findTest(const std::string& testName) const
{
	...
	if(! path.isValid())
		throw std::invalid_argument("No test named <" + testName + "> found in test <" + getName() + ">.");
	return path.getChildTest();
}

bool Test::findTestPath(const std::string& testName, TestPath& testPath) const
{
	Test* mutableThis = CPPUNIT_CONST_CAST(Test*, this);
	if(getName() == testName)
	{
...
bool Test::findTestPath(const Test* test, TestPath& testPath) const
{
	Test* mutableThis = CPPUNIT_CONST_CAST(Test*, this);
	if(this == test)
	{
